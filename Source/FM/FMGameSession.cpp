...
AFMGameSession::AFMGameSession(const class FPostConstructInitializeProperties& PCIP)
	: Super(PCIP)
{
	if (!HasAnyFlags(RF_ClassDefaultObject)){
		OnCreateSessionCompleteDelegate = FOnCreateSessionCompleteDelegate::CreateUObject(this, &AFMGameSession::OnCreateSessionComplete);
		OnDestroySessionCompleteDelegate = FOnDestroySessionCompleteDelegate::CreateUObject(this, &AFMGameSession::OnDestroySessionComplete);

		OnFindSessionsCompleteDelegate = FOnFindSessionsCompleteDelegate::CreateUObject(this, &AFMGameSession::OnFindSessionsComplete);
		OnJoinSessionCompleteDelegate = FOnJoinSessionCompleteDelegate::CreateUObject(this, &AFMGameSession::OnJoinSessionComplete);

		OnStartSessionCompleteDelegate = FOnStartSessionCompleteDelegate::CreateUObject(this, &AFMGameSession::OnStartOnlineGameComplete);
	}
}
...
*/
void AFMGameSession::CreateGameSession(int32 ControllerId){
	const FString GameType(TEXT("Type"));
	HostSession(ControllerId, GameSessionName, GameType, false, true, AFMGameSession::DEFAULT_NUM_PLAYERS);
};
...
*/
void AFMGameSession::OnStartOnlineGameComplete(FName SessionName, bool bWasSuccessful){
	IOnlineSubsystem* OnlineSub = IOnlineSubsystem::Get();
	if (OnlineSub){
		IOnlineSessionPtr Sessions = OnlineSub->GetSessionInterface();
		if (Sessions.IsValid())	{
...
/** End a pending match (this is a misnomer, basically the function means: start the match!) */
void AFMGameSession::EndPendingMatch(){
	StartGame();
}
...
* @param ControllerId id of the controller that owns this session
*/
void AFMGameSession::DestroyGameSession(int32 ControllerId){
};

bool AFMGameSession::IsBusy() const {
...
EOnlineAsyncTaskState::Type AFMGameSession::GetSearchResultStatus(int32& SearchResultIdx, int32& NumSearchResults){
	SearchResultIdx = 0;
	NumSearchResults = 0;
...
*/
const TArray<FOnlineSessionSearchResult> & AFMGameSession::GetSearchResults() const {
...
* @param bWasSuccessful true if the async action completed without error, false if there was an error
*/
void AFMGameSession::OnCreateSessionComplete(FName SessionName, bool bWasSuccessful){
...
* @param bWasSuccessful true if the async action completed without error, false if there was an error
*/
void AFMGameSession::OnDestroySessionComplete(FName SessionName, bool bWasSuccessful){
...
void AFMGameSession::DelayedSessionDelete(){
	IOnlineSubsystem* OnlineSub = IOnlineSubsystem::Get();
	if (OnlineSub){
		IOnlineSessionPtr Sessions = OnlineSub->GetSessionInterface();
		...
		if (SessionState != EOnlineSessionState::Creating){
			...
			// Retry shortly
			GetWorldTimerManager().SetTimer(this, &AFMGameSession::DelayedSessionDelete, 1.f);
		}
	}
}

bool AFMGameSession::HostSession(int32 ControllerId, FName SessionName, const FString & GameType, bool bIsLAN, bool bIsPresence, int32 MaxNumPlayers){
	IOnlineSubsystem* OnlineSub = IOnlineSubsystem::Get();
	if (OnlineSub){
		...
		IOnlineSessionPtr Sessions = OnlineSub->GetSessionInterface();
		if (Sessions.IsValid()){
			HostSettings = MakeShareable(new FFMOnlineSessionSettings(bIsLAN, bIsPresence, MaxPlayers));
...
void AFMGameSession::OnFindSessionsComplete(bool bWasSuccessful){
...
void AFMGameSession::ResetBestSessionVars(){
	CurrentSessionParams.BestSessionIdx = -1;
}

void AFMGameSession::ChooseBestSession(){
...
void AFMGameSession::StartMatchmaking(){
	ResetBestSessionVars();
	ContinueMatchmaking();
}

void AFMGameSession::ContinueMatchmaking(){
	ChooseBestSession();
...
void AFMGameSession::OnNoMatchesAvailable(){
	UE_LOG(LogOnlineGame, Verbose, TEXT("Matchmaking complete, no sessions available."));
	SearchSettings = NULL;
}

void AFMGameSession::FindSessions(int32 ControllerId, FName SessionName, bool bIsLAN, bool bIsPresence){
	IOnlineSubsystem* OnlineSub = IOnlineSubsystem::Get();
	if (OnlineSub){
		CurrentSessionParams.SessionName = SessionName;
		CurrentSessionParams.bIsLAN = bIsLAN;
		...
		IOnlineSessionPtr Sessions = OnlineSub->GetSessionInterface();
		if (Sessions.IsValid()){
			SearchSettings = MakeShareable(new FFMOnlineSearchSettings(bIsLAN, bIsPresence));
...
*/
bool AFMGameSession::JoinSession(int32 ControllerId, FName SessionName, int32 SessionIndexInSearchResults){
	bool bResult = false;
...
* @param bWasSuccessful true if the async action completed without error, false if there was an error
*/
void AFMGameSession::OnJoinSessionComplete(FName SessionName, bool bWasSuccessful){
...
bool AFMGameSession::TravelToSession(int32 ControllerId, FName SessionName){
	IOnlineSubsystem* OnlineSub = IOnlineSubsystem::Get();
	if (OnlineSub){
		FString URL;
		IOnlineSessionPtr Sessions = OnlineSub->GetSessionInterface();
